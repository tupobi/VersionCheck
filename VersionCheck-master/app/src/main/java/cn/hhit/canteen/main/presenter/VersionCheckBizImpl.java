...
import cn.hhit.canteen.app.MyApplication;
...
public class VersionCheckBizImpl implements IVersionCheckBiz {
        ...
        mIVersionCheckModel.getLatestVersion(
                ...
                new IVersionCheckModel.Callback() {
                    ...
                    public void onSuccess(AppVersion latestAppVersion) {
                        ...
                        String currentVersionName = MyApplication.getVersionName();
    ...
    public void checkNet() {
        if (NetworkUtil.isConnected(MyApplication.getGlobalApplication()) && NetworkUtil.isAvailable(MyApplication.getGlobalApplication())) {
            //网络连接上并且可用
            if (NetworkUtil.isWifiConnected(MyApplication.getGlobalApplication())) {
                //是WiFi环境
                startDownload();
            } else {
                //非WiFi环境
    ...
    public void startDownload() {
        AndPermission.with(MyApplication.getGlobalApplication())
                ...
                .onDenied(new Action() {
                    @Override
                    public void onAction(List<String> permissions) {
                        if (AndPermission.hasAlwaysDeniedPermission(MyApplication.getGlobalApplication(), permissions)) {
                            // 这些权限被用户总是拒绝。
                            final SettingService settingService = AndPermission.permissionSetting(MyApplication.getGlobalApplication());
